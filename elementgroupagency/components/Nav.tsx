...
export default function Nav() {
  ...
  // Header transparente no topo (o hero é imagem e merece o ecrã inteiro), sólido
  // a partir do primeiro scroll — antes disto o texto das páginas passava por baixo
  // do nav e lia-se através dele.
    ...
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-[background-color,backdrop-filter,border-color] duration-300 ${
        scrolled
          ...
          : 'bg-gradient-to-b from-bg/70 to-transparent backdrop-blur-[2px] border-b border-transparent'
      }`}
    >
      <div className="max-w-[1200px] mx-auto px-6 h-20 flex items-center justify-between">
          ...
          {NAV_LINKS.map((link) => {
            ...
            return (
              <Link
                ...
                className={`relative ${linkCls} ${active ? 'text-white' : ''}`}
              >
                {link.label}
                {active && <span aria-hidden className="absolute -bottom-1.5 left-0 right-0 h-px bg-accent" />}
              </Link>
            )
          })}
        </nav>
        ...
        <Link
          ...
          className="hidden md:inline-flex items-center justify-center rounded-pill bg-white text-black
                     text-sm font-medium px-6 py-2.5 transition-all duration-300
